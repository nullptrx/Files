...
class RemoteInputStream : InputStream, Parcelable {
  ...
  override fun skip(size: Long): Long =
    remoteInputStream?.call { exception -> skip(size, exception) } ?: localInputStream!!.skip(size)

  @Throws(IOException::class)
  override fun available(): Int =
    remoteInputStream?.call { exception -> available(exception) } ?: localInputStream!!.available()
